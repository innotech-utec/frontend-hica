<template>
    <v-container>
      ...
      <v-form @submit.prevent="updateFactura">
        ...
        <v-btn color="primary" type="submit">Actualizar Factura</v-btn>
      </v-form>
    </v-container>
  </template>
  ...
  export default {
    ...
    methods: {
      async fetchFactura() {
        const response = await backend.get(`/facturas/${this.$route.params.id}`);
        const factura = response.data;
        this.fichaClinicaId = factura.fichaClinicaId;
        this.articulosSeleccionados = factura.articulos.map((articulo) => ({
          id: articulo.id,
          cantidad: articulo.FacturaArticulo.cantidad
        }));
      },
      ...
      async updateFactura() {
        try {
          await backend.patch(`/facturas/${this.$route.params.id}`, {
